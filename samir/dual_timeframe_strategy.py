    ...
    def generate_entry_signal(self, am, bar, window_direction):
        ...
        if window_direction == "long":
            if fast[-2] < slow[-2] and fast[-1] > slow[-1]:
                self.owner.output_msg(
                    f"[generate_entry_signal] signal long - fast[-2]: {fast[-2]}, slow[-2]: {slow[-2]}, fast[-1]: {fast[-1]}, slow[-1]: {slow[-1]}"
                )
                return "long"
        elif window_direction == "short":
            if fast[-2] > slow[-2] and fast[-1] < slow[-1]:
                self.owner.output_msg(
                    f"[generate_entry_signal] signal short - fast[-2]: {fast[-2]}, slow[-2]: {slow[-2]}, fast[-1]: {fast[-1]}, slow[-1]: {slow[-1]}"
                )
                return "short"
        return None
...
class DualStrategy(CtaTemplate):
    ...
    def __init__(self, cta_engine, strategy_name: str, vt_symbol: str, setting: dict):
        ...
        self.short_signal_strategy: ShortSignalStrategy = EMA_CrossShort(
            owner=self, fast=self.short_fast_ema, slow=self.short_slow_ema
        )
    ...
    def on_long_bar(self, bar: BarData):
        ...
        if signal == "close_long" and self.pos_state.direction == "long":
            ...
            self.output_msg(
                f"[on_long_bar] generate pending_windows: direction={signal} start_bar={self.long_bar_count}"
            )
        ...
        for side, w in self.pending_windows.items():
            if self.long_bar_count >= w.long_end:
                expired.append(side)
                self.output_msg(
                    f"[on_long_bar] expired.append bar: direction={w.direction} start_bar={w.start_bar} long_end={w.long_end}"
                )
        for k in expired:
    ...
    def update_capital(self, trade: TradeData):
        ...
        if trade.direction == Direction.LONG:
            ...
            self.leverage_capital += trade_amount - commission - slippage_cost
        if(self.pos == 0):
            self.current_capital = self.cta_engine.capital + self.leverage_capital - self.cta_engine.capital * self.leverage_ratio
    ...
    def cover_pos(self):
        ...
        self.output_msg(f"[cover_pos] capital = {self.current_capital:.2f},leverage_capital = {self.leverage_capital:.2f}")
